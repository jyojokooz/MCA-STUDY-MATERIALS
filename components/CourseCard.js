function CourseCard({ course, onViewDetails }) {
  ...
  try {
    ...
    return (
      <div 
        className={`rounded-xl p-6 border-3 shadow-[6px_6px_0px_0px_rgba(0,0,0,0.9)] hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,0.9)] transition-all duration-200 relative ${
          isDarkMode 
        ...
        <div className="space-y-2 mb-4">
          ...
          {course.marks && (
            <div className="flex justify-between text-sm">
              ...
              <span className={`font-black ${
                isDarkMode ? 'text-white' : 'text-black'
              }`}>IA:{course.marks.ia} ESE:{course.marks.ese}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between">
          {course.examSlot && (
            <span className={`text-xs ${
              isDarkMode ? 'text-gray-400' : 'text-gray-500'
            }`}>Exam Slot: {course.examSlot}</span>
          )}
          <div className="flex items-center gap-2">
            ...
            } ${getTypeColor(course.type)} text-white`}>
              {course.semester}
            </span>
          </div>
        </div>
